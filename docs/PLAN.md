...
`jj`는 이 반복 비용을 줄이기 위한 local-first CLI입니다. 사용자가 한 번 작성한 `docs/PLAN.md`를 시작점으로 삼고, 이후에는 `.jj/spec.json`과 `.jj/tasks.json`을 현재 workspace의 실행 가능한 제품 상태로 유지하면서 Codex 실행, deterministic validation, 증거 기록까지 하나의 재현 가능한 흐름으로 묶습니다.
...
- 사용자는 대시보드를 시작점으로 README, `docs/PLAN.md`, project docs, `.jj/spec.json`, `.jj/tasks.json`, `.jj/runs` 아티팩트를 검토할 수 있다.
