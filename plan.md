...
`jj`는 이 반복 비용을 줄이기 위한 local-first CLI입니다. 사용자가 한 번 작성한 `plan.md`를 시작점으로 삼고, 이후에는 `.jj/spec.json`과 `.jj/tasks.json`을 현재 workspace의 실행 가능한 제품 상태로 유지하면서 Codex 실행, deterministic validation, 증거 기록까지 하나의 재현 가능한 흐름으로 묶습니다.

## 문서와 상태의 역할

- `README.md`, `plan.md`, `docs/PRD.md`, `docs/SPEC.md`, `docs/TASK.md`는 사람이 읽는 제품 경계와 개발 문서다.
- `.jj/spec.json`은 bootstrap 이후 planning source of truth인 현재 SPEC이다.
- `.jj/tasks.json`은 run마다 append되는 workspace task proposal history다.
- `.jj/runs/<run-id>/`는 manifest, snapshots, logs, validation, summaries를 담는 local run evidence다.
- `.jj/documents.sqlite3`는 redacted `.jj/` 문서를 모아 두는 local SQLite mirror다. 이 DB는 검색과 이력 보관을 위한 파생물이며, `.jj/spec.json`이나 `.jj/tasks.json`을 대체하지 않는다.
- `docs/` Markdown은 current product boundary 문서지만, current runs가 자동으로 갱신하는 runtime state가 아니다.

## 목표

- 하나의 기획 seed에서 canonical JSON SPEC과 append-only task history를 만들고 유지한다.
...
- `jj serve`로 product docs, JSON state, run artifacts를 guarded local dashboard에서 볼 수 있게 한다.
- 모든 개발이 written intent에서 시작하고 JSON state, validation, git diff, run evidence로 검토되도록 한다.
...
2. 사용자가 `jj run plan.md --dry-run`으로 planning preview와 snapshots를 먼저 확인할 수 있다.
3. 사용자가 `jj run plan.md`를 실행한다.
4. `jj`가 입력 파일, current `.jj/spec.json`, `.jj/tasks.json` 요약, 최근 run evidence, git baseline을 기록한다.
5. Planner가 fresh task batch를 만들고 `.jj/tasks.json`에 append한다.
...
8. validation이 통과하면 선택 task를 `done`으로 표시하고 결과 기반으로 `.jj/spec.json`을 reconcile한다.
9. validation이 실패, 누락, skipped이면 기존 `.jj/spec.json`을 유지한다.
10. 사용자가 `jj serve --cwd .`의 대시보드에서 product docs, JSON state, task 상태, run evidence, validation 상태를 검토한다.
...
- `--dry-run`은 planning artifact와 state snapshot만 `.jj/runs/<run-id>/`에 만들고 workspace `.jj/spec.json`/`.jj/tasks.json` 쓰기와 구현 Codex 실행을 하지 않는다.
- full run은 planning 중 `.jj/tasks.json`에 새 task를 append하고, validation 성공 후에만 `.jj/spec.json`을 갱신한다.
...
- `jj serve`는 첫 화면을 대시보드로 제공하고, allowlisted product docs, JSON state, run artifact를 로컬 HTTP 서버로 탐색하게 한다.
- 대시보드는 현재 JSON task 상태, 진행 중인 run, 최근 run status, validation 결과, 실패/위험 항목, 다음 액션을 보여준다.
- 구현 변경은 관련 public docs 또는 JSON state와 실제 동작이 일치해야 완료로 본다.
...
- 사용자가 `jj run plan.md --dry-run`을 실행하고 `.jj/runs` 아래의 planning artifacts와 state snapshots를 확인할 수 있다.
...
- 사용자는 대시보드를 시작점으로 README, `plan.md`, project docs, `.jj/spec.json`, `.jj/tasks.json`, `.jj/runs` 아티팩트를 검토할 수 있다.
- 변경된 기능은 문서, JSON state, 구현이 함께 맞아떨어지도록 검토되어 document-first 개발 흐름을 유지한다.
