...
9. 사용자가 `jj serve --cwd .`로 README, 실행 아티팩트, 생성 문서, manifest를 검토한다.
...
- `jj serve`는 Markdown 문서와 run artifact를 로컬 HTTP 서버로 제공한다.
...
- `jj serve --cwd .`는 README, `plan.md`, project docs, `.jj/runs` 아티팩트를 secret 노출 없이 보여준다.
